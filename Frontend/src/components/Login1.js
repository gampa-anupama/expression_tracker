  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(""); // Clear the error message if registration is successful
      } else {
        setError(response.data.message); // Set the error message if something went wrong
        setMessage(""); // Clear the success message if there is an error
      }
    } catch (error) {
...
<div>
  {message && <p className="success-message">{message}</p>}  {/* Show success message if `message` is not empty */}
  {error && <p className="error-message">{error}</p>}        {/* Show error message if `error` is not empty */}
</div>
